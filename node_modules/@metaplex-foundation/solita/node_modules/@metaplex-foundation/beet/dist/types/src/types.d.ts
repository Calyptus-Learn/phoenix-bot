 ...
 * Enum Variant Kinds
 */
export declare type DataEnumKind<T> = keyof T;
/**
 * Turns a `Record<K, Beet<V>>` into a discriminated union `{ __kind: K, dataBeet: Beet<V> }`.
 */
export declare type DataEnumBeet<T, Kind extends DataEnumKind<T> = DataEnumKind<T>> = [
    Kind,
 ...
 */
export declare type DataEnumKeyAsKind<T> = {
    [K in DataEnumKind<T>]: {
        __kind: K;
    } & T[K];
}[keyof T];
